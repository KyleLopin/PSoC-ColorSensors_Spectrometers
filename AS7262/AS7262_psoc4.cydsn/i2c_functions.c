...
uint8 sensor_write(uint8 address, uint8* buffer, uint8 num_bytes) {   
    uint8 status = 0;
    status = I2C_I2CMasterWriteBuf(address, buffer, num_bytes, I2C_I2C_MODE_COMPLETE_XFER);
    for(;;) {
        if (0x00 != (I2C_I2CMasterStatus() & I2C_I2C_MSTAT_WR_CMPLT)) {
            break;
        }
    }
    return status;
...
//    LCD_PrintString("read0020   ");
    ...
    temp = I2C_I2CMasterWriteBuf(address, &_register, 1, I2C_I2C_MODE_NO_STOP);
//    LCD_Position(0, 0); 
...
//        LCD_PrintString(LCD_str);
    ...
    do {
        temp = I2C_I2CMasterReadBuf(address, buffer, num_bytes, I2C_I2C_MODE_REPEAT_START);
        for(;;) {
            if (0x00 != (I2C_I2CMasterStatus() & I2C_I2C_MSTAT_RD_CMPLT)) {
                break;
            }
        }
    }
    while (temp != I2C_I2C_MSTR_NO_ERROR);
